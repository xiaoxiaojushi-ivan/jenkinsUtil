...
function getFbDB(){
    $.ajax({
        type : "get",
        async:false,
        url : "gktDB/getGktDB?page="+currentPage+"&pageCapacity="+pageCapacity+"&creator="+creator+"&environment="+env+"&sortName=creation_time&sortType=desc",
        dataType : "json",
        beforeSend: LoadFunction,
        error: erryFunction,
        success: succFunction
    ...
    function succFunction(res) {
        ...
        $.each(datas, function (i, data) {
            ...
            var build_version = undefinedFormat(data.build_version);
            var base_build_num = undefinedFormat(data.base_build_num);
            var base_image_version = undefinedFormat(data.base_image_version);
            var publish_image_to_aws = undefinedFormat(data.publish_image_to_aws);
            var components = data.components;
            var project_name= upperCase(components);
...
//            var components_size = componentsSize(data.components); 
            ...
            var offImages = splitImages(data.offlineImages);
            var onImages = splitImages(data.onlineImages);
            var firstOffImage = getFirstImage(data.offlineImages);
            var firstOnImage = getFirstImage(data.onlineImages);
...
//                        "</span>"
//                +"</td>"
*/                +"<td><a href=\"http://172.30.10.171/GKT/"+project_name+"/snapshots/"+build_version+"\">repository</a></td>"
